...
if app_view == "Entry":
    ...
    # Clean sliders with integrated labels (no duplicate)
    w_val = float(last['Weight (kg)'])
    w = st.slider("Weight (kg)", min_value=max(0.0, w_val-5.0), max_value=w_val+5.0, value=w_val, step=0.1,
                  help="Adjust your current weight")

    mm_mode = st.session_state.get('muscle_mass_input_mode', 'Percentage (%)')
    if mm_mode == "Kilograms (kg)":
        m_val = float(last['Muscle Mass (kg)'])
        m = st.slider("Muscle Mass (kg)", min_value=max(0.0, m_val-5.0), max_value=m_val+5.0, value=m_val, step=0.1,
                      help="Adjust your current muscle mass")
    else:
        ...
        m_pct = st.slider("Muscle Mass (%)", min_value=max(0.0, current_pct-5.0), max_value=min(100.0, current_pct+5.0), value=current_pct, step=0.1,
                          help="Adjust your current muscle mass percentage")
        m = w * (m_pct / 100.0)
    ...
    bf = st.slider("Body Fat (%)", min_value=max(3.0, bf_val-5.0), max_value=bf_val+5.0, value=bf_val, step=0.1,
                   help="Adjust your current body fat percentage")
